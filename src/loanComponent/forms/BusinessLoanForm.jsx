...
// // New Reusable Component for Business Loan Form - Will replace the above component in the future after testing and validation
...
//           }
//         })
//         .catch(() => toast.error("Unable to fetch location details"));
//     }
//   }, [form.pincode]);

//   // Simplified eligibility for Business Loan
//   const evaluateEligibility = () => {
//     const employmentKey = employmentTypeMap[form.employmentType] || "";
...
//   // Auto-check eligibility → submit → show result
//   useEffect(() => {
//     if (step === 6 && !eligibilityResult && !loading) {
//       const timer = setTimeout(async () => {
//         const result = evaluateEligibility();
//         setEligibilityResult(result);
//         await submitFinalLoan(result); // submit anyway (with eligible flag)
//         setStep(7);
//       }, 5000); // longer delay for UX feel
...
//       } else {
//         toast.error(result?.message || "Submission failed");
//         setStep(3); // fallback to employment type select
//       }
//     } catch {
//       toast.error("Server error");
//       setStep(3);
...
//   const handleStep1Submit = async () => await createLoan();
...
//     if (type === "Self-employed Professional" && !form.itr) {
//       toast.error("ITR is mandatory for Self-Employed Professional");
//       return;
//     }
//     if (type === "Self-employed Business" && (!form.itr || !form.gst)) {
//       toast.error("ITR & GST are mandatory for Self-Employed Business");
//       return;
//     }
...
//                 Choose Repayment Tenure (in months)
//               </label>
//               <div className="flex gap-5">
//                 {[12, 24, 36, 60].map((m) => (
//                   <SquareOption
//                     key={m}
//                     label={m}
//                     active={form.tenure === m.toString()}
//                     onClick={() =>
//                       setForm((p) => ({ ...p, tenure: m.toString() }))
//                     }
//                   />
//                 ))}
//               </div>
...
//             {["Self-employed Professional", "Self-employed Business"].map(
//               (type) => (
//                 <OptionCard
//                   key={type}
//                   title={
...
//                   onClick={() => {
//                     setForm((p) => ({ ...p, employmentType: type }));
//                     setStep(5);
//                   }}
//                 />
//               )
...
//                 onChange={(val) =>
//                   setForm((p) => ({ ...p, employmentDuration: val }))
//                 }
...
//             <NotEligibleCard />
//           ))}
//         {step === 8 && <EligibleSuccessCard />}{" "}
//         {/* or custom success if needed */}
//       </div>
//     </div>
//   );
// }
...
export default function BusinessLoanForm() {
  return <PersonalLoanForm loanType="Business-Loan" />;
}
