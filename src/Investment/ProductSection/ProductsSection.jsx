...
/* -----------------------------------------------
   CONFIG (Centralized & Future-safe)
----------------------------------------------- */
const STRAPI_URL = process.env.NEXT_PUBLIC_STRAPI_URL || "";

// fallback image (UI never breaks)
const FALLBACK_IMAGE = "/Investment/gc1.svg";

/* -----------------------------------------------
   COMPONENT
----------------------------------------------- */
export default function ProductsSection({ products = [], headers }) {
  /* -----------------------------------------------
     NORMALIZE DATA (IMPORTANT FOR SCALABILITY)
  ----------------------------------------------- */
  ...
  return (
    <div className="bg-white px-4 sm:px-8 md:px-16 lg:px-[136px] pb-10 lg:pb-20">
      <section className="flex flex-col items-center gap-8 sm:gap-12">
        ...
        {/* HEADER (Dynamic) */}
        <h2 className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-bold text-gray-700 leading-relaxed tracking-wide text-center md:text-left">
          <span className="text-gray-700">{headers?.title1} </span>
          <span className="text-yellow-400">{headers?.title2} </span>
          <span className="text-gray-700">{headers?.yellowText} </span>
          <span className="text-yellow-400">{headers?.title4}</span>
        </h2>
