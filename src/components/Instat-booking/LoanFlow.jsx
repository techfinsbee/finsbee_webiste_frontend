...
const TWO_FACTOR_API_KEY = process.env.NEXT_PUBLIC_TWO_FACTOR_API_KEY;
...
export default function LoanFlow() {
  ...
  // ── Your original OTP send function (adapted) ────────────────────────
  const sendOtp = async () => {
    ...
    try {
      const url = `/twofactor/API/V1/${TWO_FACTOR_API_KEY}/SMS/+91${trimmed}/AUTOGEN/OTP%20Verify?var1=${trimmed}`;
  ...
  // ── Your original OTP verify function (adapted) ──────────────────────
  const verifyOtp = async () => {
    if (!otpRequestId) {
      alert("No OTP request found. Please try sending again.");
      return;
    }
    ...
    try {
      const verifyUrl = `/twofactor/API/V1/${TWO_FACTOR_API_KEY}/SMS/VERIFY/${otpRequestId}/${otp}`;
  ...
  const handleContinue = async () => {
    ...
    if (!otpVerified) {
      // Trigger OTP only if not already verified
      const sent = await sendOtp();
      if (!sent) return;
    } else {
      // Already verified → directly submit
      handleContinueAfterOtp();
    }
  };
  ...
  const handleContinueAfterOtp = async () => {
    try {
      ...
      const resultArray = Array.isArray(response?.result)
        ? response.result.filter((item) => item && item.lender)
        : [];
  ...
  const getRecommendedVendors = () => {
    ...
    const resultArray = Array.isArray(apiResponse?.result)
      ? apiResponse.result.filter((item) => item && item.lender)
      : [];
    ...
    resultArray.forEach((item) => {
      const lenderName = item.lender?.toLowerCase();

      const vendorId = lenderMapping[lenderName] || lenderName;
  ...
  const inputClass = (field) =>
    `w-full py-3 pl-10 pr-4 rounded-md border focus:ring-2 focus:ring-yellow-200 focus:outline-none placeholder-gray-400 ${
      ...
      field === "phone" && otpVerified ? "bg-gray-100 cursor-not-allowed" : ""
    }`;
    ...
    <div className="min-h-screen relative bg-[#FAFAFA] flex flex-col items-center pt-12">
      {/* {showOtpModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-xl w-[350px] shadow-lg">
            ...
            <input
              ...
              className="w-full border p-3 rounded-md mb-4 text-center tracking-widest text-2xl"
              maxLength={6}
              autoFocus
            />
            ...
            <div className="flex flex-col gap-3">
              <button
                onClick={sendOtp}
                disabled={otpLoading}
                className="w-full bg-gray-200 py-2 rounded disabled:opacity-50"
              >
              ...
              <button
                ...
                className="w-full bg-yellow-400 py-2 rounded font-semibold disabled:opacity-50"
              >
      ...
      {showOtpModal && (
        ...
        >
          <div
            className="bg-white p-8 rounded-xl w-[350px] shadow-lg"
          ...
          >
            ...
            <input
              ...
              value={otp}
              onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))}
              className="w-full border p-3 rounded-md mb-4 text-center tracking-[10px] text-xl outline-none focus:border-yellow-400"
              maxLength={6}
              autoFocus
            />
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                onClick={verifyOtp}
                disabled={otpLoading || otp.length !== 6}
                className="w-full bg-yellow-400 py-2 rounded font-semibold hover:bg-yellow-500 disabled:opacity-50"
              >
                {otpLoading ? "Verifying..." : "Verify OTP"}
              </button>
      ...
      <div className="z-10 w-full flex justify-center">
            ...
            <div className="mb-5 flex gap-4">
              <div className="w-1/2 relative">
                <div className="relative">
                  <input
                    type="tel"
                    placeholder="click to verify phone number"
                    className={inputClass("phone")}
                    value={phone}
                  ...
                  <span className="absolute left-3 top-[32%] -translate-y-1/2 text-gray-400 pointer-events-none">
                    <img
                      src="/form_page/call.svg"
                      alt="Phone Icon"
                      className="w-5 h-5"
            ...
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-lg p-8 border border-yellow-100">
              ...
              <div className="flex flex-col gap-5">
                {recommendedVendors.map((vendor) => {
                  ...
                  const vendorResult = resultArray.find(
                    (r) => r?.lender?.toLowerCase() === vendor.id.toLowerCase()
                  );
                  ...
                  const handleVendorClick = async () => {
                    try {
                      if (lineId) {
                        await fetch("/api/flutterapi/loanremark/update", {
                          method: "POST",
        ...
        {/* Thank you step is commented in your code - kept same */}
      </div>
    </div>
  );
}
