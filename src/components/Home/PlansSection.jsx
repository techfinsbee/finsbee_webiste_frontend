...
const ExtraStyles = () => (
  <style>{`
    ...
    /* Bullets that always show (white on purple, black on yellow/white) */
    ...
    >
      ...
      >
        ...
        <ul className="mt-3 space-y-2 text-sm">
          {hover.points.map((p) => (
            <li key={p} className="bullet">{p}</li>
          ))}
        </ul>
      </div>
    </div>
    ...
    <section id="plans" className="relative z-0">
      ...
      <div className="absolute inset-0 -z-10 pointer-events-none">
        <div
          ...
          style={{
            ...
            backgroundSize: bgFit === "actual" ? "auto" : bgFit === "contain" ? "contain" : "cover",
          }}
        />
        {overlayOpacity > 0 && (
          <div aria-hidden className="absolute inset-0" style={{ background: overlay, opacity: overlayOpacity }} />
        )}
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 lg:py-20">
        ...
        <div className="text-center mb-10">
          <h2 className="text-2xl sm:text-3xl font-semibold text-black">
            <span className="relative inline-block mx-auto">
              Our{" "}
              <a href="#plans" className="text-[#592eff] no-underline">Plans</a>
              <span aria-hidden className="block mx-auto mt-2 w-[169px] h-0 border-b-[4px] border-[#ffc73c]" />
            </span>
          </h2>
        </div>
        ...
        <div className="md:hidden -mx-4">
          ...
          >
            {plans.map((p) => (
              <PlanCard key={p.id} {...p} className="snap-center" featured={!!p.badge} />
            ))}
          </div>
        </div>
        ...
        <div ref={desktopRef} className="hidden md:flex justify-center gap-5 lg:gap-7">
          {plans.map((p) => (
            <PlanCard
              ...
              {...p}
              featured={!!p.badge}
              innerRef={(el) => el && el.setAttribute("data-animate", p.animateFrom)}
            />
          ))}
        </div>
      </div>
